...
(function () {
  ...
  window.spawnFloatingText = function (x, y, text, color, followPlayer = false) {
      let p = window.player;
  ...
  window.renderLightingOverlay = function (mainCtx, mainCanvas) {
      ...
      // 1. Fill entire lightmap with pure White (#ffffff = 1.0 = No change in Multiply Mode)
      ...
      // 2. Restrict Ambient Darkness strictly to Active Map World Bounds
      let mapPixelW = map.width * map.tileSize;
      let mapPixelH = map.height * map.tileSize;

      lightingCtx.fillStyle = ambientColor;
      lightingCtx.fillRect(0, 0, mapPixelW, mapPixelH);
  ...
  window.triggerExtraction = function (success = true, isAbandon = false) {
      ...
      if (!success && !isAbandon) {
        ...
        if (killerNameEl) {
          killerNameEl.innerText = (window.playerStats.killedBy || "UNKNOWN FOE").toUpperCase();
        }
        setTimeout(() => {
          if (window.RenderEngine && window.RenderEngine.renderNemesisPreview) {
            window.RenderEngine.renderNemesisPreview(window.playerStats.killedByMob);
          }
        }, 50);
      } else {
    ...
    if (p.isMoving) {
        ...
        if (p.levelUpTimer && p.levelUpTimer > 0) {
          ...
          if (p.levelUpTimer % 22 === 0 && window.combatVisuals) {
            window.combatVisuals.spawnParticles(p.x, p.y - 8, 12, "gold_dungeon", 3);
          }
        }
    ...
    // Real-Time Camera & Line-Of-Sight Viewport Tracker
            ...
            if (map && map.extractionTile && map.exploredGrid) {
              ...
              if (map.exploredGrid[pTileY] && map.exploredGrid[pTileY][pTileX]) {
                ...
                if (portalPx >= minX && portalPx <= maxX && portalPy >= minY && portalPy <= maxY) {
        ...
        if (tile === window.TILE_TYPES.CHEST_SPAWN) {
          ...
          if (Math.random() < 0.4) {
            ...
            let newItem = window.createItemObject(
                        ...
                        let isEquipped = window.tryAutoEquip ? window.tryAutoEquip(newItem) : false;
                        if (!isEquipped) {
                          p.bag.push(newItem);
                        }
                        window.updateHUD();
        ...
        if (m.hp.lte(0)) {
          ...
          if (Math.random() < 0.05) {
            ...
            let droppedItem = window.createItemObject(
                        ...
                        let isEquipped = window.tryAutoEquip ? window.tryAutoEquip(droppedItem) : false;
                        if (!isEquipped) {
                          p.bag.push(droppedItem);
                        }
                        window.updateHUD();
                        if (typeof window.pushToast === "function")
                          window.pushToast(droppedItem);
          }
  ...
  // --- RENDER ENGINE ---
    ...
    // 2. Y-Sorted Depth Queue (Structures, Mobs, Boss, and Hero)
        ...
        if (window.currentGameState === window.GAME_STATES.HUB && mapInst && mapInst.stations) {
          mapInst.stations.forEach((st) => {
            depthQueue.push({
              yBase: st.y * tSize + 24,
              draw: () => {
                if (window.drawDungeonStructureTile) {
                  window.drawDungeonStructureTile(ctx, st.type, st.x * tSize, st.y * tSize, tSize);
                }
              },
            });
          });
        ...
        // B. Chest Spawns (Dungeon State)
        if (window.currentGameState !== window.GAME_STATES.HUB && mapInst && mapInst.grid) {
          let camera = window.DungeonCamera;
          let startCol = Math.max(0, Math.floor(camera.x / tSize));
          let endCol = Math.min(mapInst.width - 1, Math.ceil((camera.x + camera.viewportW / camera.zoom) / tSize));
          let startRow = Math.max(0, Math.floor(camera.y / tSize));
          let endRow = Math.min(mapInst.height - 1, Math.ceil((camera.y + camera.viewportH / camera.zoom) / tSize));
            ...
            for (let c = startCol; c <= endCol; c++) {
              if (mapInst.grid[r][c] === window.TILE_TYPES.CHEST_SPAWN) {
                ...
                depthQueue.push({
                  ...
                  draw: () => {
                    if (window.drawDungeonStructureTile) {
                      window.drawDungeonStructureTile(ctx, window.TILE_TYPES.CHEST_SPAWN, px, py, tSize);
                    }
                  },
                });
              }
        ...
        // E. Player Hero
        let bounce = 0;
        let deathFrame = window.deathAnimationTimer > 0 ? 75 - window.deathAnimationTimer : 0;
        if (deathFrame > 0) {
        ...
        // Floating System Text Effects
            window.floatingTexts.forEach((ft) => {
              ...
              let drawX = ft.followPlayer && window.player ? window.player.x + (ft.offsetX || 0) : ft.x;
              let drawY = ft.followPlayer && window.player ? window.player.y + (ft.offsetY || 0) : ft.y;
  ...
  window.UIManager = window.UIManager || {};
    ...
    // --- HIDE TOOLTIPS WITH DESKTOP GRACE PERIOD & IMMEDIATE DISMISS ---
      ...
      window.hideTooltip = (immediate = false) => window.UIManager.hideTooltip(immediate);
    ...
    window.UIManager.positionTooltip = function (e, tt) {
      ...
      let container = containerEl ? containerEl.getBoundingClientRect() : { left: 0, top: 0 };
      ...
      if (isMobile) {
        ...
        if (isComparison) {
          ...
          tt.querySelectorAll(".tooltip-card").forEach((card) => { card.style.padding = "6px 8px"; });
          tt.querySelectorAll(".tt-title").forEach((title) => { title.style.fontSize = "10.5px"; title.style.marginBottom = "2px"; });
          tt.querySelectorAll(".tt-subtitle").forEach((sub) => { sub.style.fontSize = "8.5px"; sub.style.marginBottom = "2px"; });
          tt.querySelectorAll(".tt-stat-line").forEach((line) => { line.style.fontSize = "9px"; line.style.marginBottom = "1px"; });
    ...
    window.showItemTooltip = function (e, item) {
      ...
      tt.style.borderColor = window.getTierColor ? window.getTierColor(item.statsRolled) : "#3498db";
      tt.style.display = "block";
      window.positionTooltip(e, tt);
    };

    window.showInventoryTooltip = function (e, itemId) {
      if (e && e.target && e.target.closest && (e.target.closest("button") || e.target.closest(".btn-action"))) return;
      e.stopPropagation();

      let item =
        (window.inventory && window.inventory.EQUIP && window.inventory.EQUIP.find((i) => i.id === itemId)) ||
        (window.inventory && window.inventory.ARTIFACT && window.inventory.ARTIFACT.find((i) => i.id === itemId)) ||
        (window.inventory && window.inventory.SIGIL && window.inventory.SIGIL.find((i) => i.id === itemId)) ||
    ...
    window.toggleRingComparisonSlot = function (e, itemId) {
      ...
      window.state.preferredRingComparisonSlot =
        (window.state.preferredRingComparisonSlot || "ring1") === "ring1" ? "ring2" : "ring1";
  ...
  window.renderProfileModal = function () {
    ...
    if (matrixGridEl) {
          ...
          let renderAttrCard = (name, desc, attrKey, committedCount, stagedCount, iconType) => {
            let totalCount = committedCount + stagedCount;
            let stagedBadge = stagedCount > 0 ? `<span class="attr-staged-val">(+${stagedCount})</span>` : "";
            let sub1Disabled = stagedCount < 1 ? "disabled" : "";
            let isStaged = stagedCount > 0;
            let iconSvg = typeof window.getUiIconSvg === "function" ? window.getUiIconSvg(iconType, 13) : "";
    ...
    // 3. Render Right Panel (Equipment Satchel)
        let displayList = isHub
          ? window.inventory.EQUIP || []
          : window.player.bag || [];
        let sectionHeader = isHub ? "EQUIPMENT SATCHEL" : "CARRIED RUN SATCHEL";
...
window.tryAutoEquip = function (item) {
    ...
    if (type === "weapon" || type === "helmet" || type === "boots") {
    ...
    } else if (type === "subweapon" || type === "shield" || type === "dagger" || type === "tome") {
      ...
      let isAlreadyEquipped = ["art1", "art2", "art3"].some(
        (s) => window.equippedSlots[s] && window.equippedSlots[s].trait === item.trait
      );
  ...
  // --- LOOT SATCHEL & VAULT TOGGLE ---
    window.toggleLootBag = function () {
      ...
      if (headerEl) {
        headerEl.innerHTML = `EXTRACTED LOOT SATCHEL <span style="font-size:10px; color:#94a3b8; font-family:monospace; margin-left:6px;">(${displayList.length} / ${maxBag})</span>`;
      }

      if (modal.style.display === "none" || modal.style.display === "") {
        if (displayList.length === 0) {
          listEl.innerHTML = `<div style="font-size:10.5px; color:#64748b; font-style:italic; padding:24px; text-align:center; background:rgba(0,0,0,0.3); border:1px dashed #1e293b; border-radius:6px;">Satchel is empty.<br>Defeat monsters and open treasure chests in the dungeon to gather loot!</div>`;
        } else {
          listEl.innerHTML = displayList
            .map((item, idx) => {
              ...
              let statPreview = [];
              if (item.atk) statPreview.push(`ATK +${window.formatNumber ? window.formatNumber(item.atk) : item.atk}`);
              if (item.def) statPreview.push(`DEF +${window.formatNumber ? window.formatNumber(item.def) : item.def}`);
              if (item.maxHp) statPreview.push(`HP +${window.formatNumber ? window.formatNumber(item.maxHp) : item.maxHp}`);
              let statStr =
                statPreview.length > 0 ? statPreview.join(" | ") : `${starsLabel}`;

              let insureBtn = `<button class="action-btn-sm ${isInsured ? "action-btn-insured" : "action-btn-insure"}" onclick="event.stopPropagation(); window.toggleInsurance(${item.id}); window.toggleLootBag(); window.toggleLootBag();">${isInsured ? "[INSURED]" : "INSURE"}</button>`;
  ...
  window.pushToast = function (item) {
    ...
    let autoEquippedBadge = item.wasAutoEquipped
          ? `<span style="background:#2ecc71; color:#05030a; font-weight:900; font-size:7.5px; padding:1px 4px; border-radius:3px; font-family:monospace; margin-left:5px; letter-spacing:0.5px;">AUTO-EQUIPPED</span>`
          : "";

        toast.innerHTML = `
                  ${iconHtml}
                  <div class="toast-info">
                    <div class="toast-title" style="color:${col}; display:flex; align-items:center; flex-wrap:wrap;">${item.name}${autoEquippedBadge}</div>

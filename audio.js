  ...
  init() {
    ...
    if (navigator.audioSession) {
      try {
        ...
        navigator.audioSession.type = settings.mixWithBackground
          ? "ambient"
          : "playback";
      } catch (e) {
...
window.MusicManager = {
  ...
  synthesizePianoPing(time) {
    ...
    const baseNote =
      this.activeChordNotes[
        Math.floor(Math.random() * this.activeChordNotes.length)
      ];
  ...
  updateBGMState() {
    ...
    if (window.currentGameState === states.DUNGEON) {
      ...
      const hasBoss =
        window.mob && window.mob.hp && typeof window.mob.hp.gt === "function"
          ? window.mob.hp.gt(0)
          : window.mob && window.mob.hp > 0;
      ...
      if (p && p.hp !== undefined && p.maxHp !== undefined) {
        let hpRatio = 1.0;
        if (
          typeof p.hp.toNumber === "function" &&
          typeof p.maxHp.toNumber === "function"
        ) {
  ...
  playProceduralPad(time) {
    ...
    const chords = [
      { notes: [130.81, 164.81, 196.0, 261.63], drone: 65.41 }, // C Major Chord
      { notes: [146.83, 174.61, 220.0, 293.66], drone: 73.42 }, // D minor Chord
      { notes: [164.81, 196.0, 246.94, 329.63], drone: 82.41 }, // E minor Chord
      { notes: [174.61, 220.0, 261.63, 349.23], drone: 87.31 }, // F Major Chord
    ];
  ...
  synthesizeSnare(time) {
    if (
      !this.ctx ||
      !window.SoundManager ||
      !window.SoundManager.cachedNoiseBuffer
    )
      return;

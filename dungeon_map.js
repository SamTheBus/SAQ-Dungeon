...
(function () {
  class DungeonMapGenerator {
      ...
      let connected = new Set([0]);
            ...
            for (let i = 0; i < this.rooms.length; i++) {
              for (let j = i + 1; j < this.rooms.length; j++) {
                let dist = Math.hypot(this.rooms[i].cx - this.rooms[j].cx, this.rooms[i].cy - this.rooms[j].cy);
    ...
    placeSpawnAndExtraction() {
          ...
          let topCandidates = candidateRooms.slice(0, Math.min(3, candidateRooms.length));
          let chosen = topCandidates[Math.floor(Math.random() * topCandidates.length)].room;
    ...
    revealSightRadius(px, py, pInt = 0) {
          ...
          let radius = Math.min(12, 7 + Math.floor(effectiveInt / 30));
...
window.drawDungeonPortalTile = function (ctx, tileType, cx, cy, tileSize) {
  ...
  ctx.setLineDash([4, 4]);
  ctx.beginPath();
  ctx.ellipse(0, 0, tileSize * 0.35, tileSize * 0.22, Math.PI / 4, 0, Math.PI * 2);
  ctx.stroke();
  ctx.restore();
  ...
  ctx.beginPath();
  ctx.arc(cx, cy, tileSize * 0.12 + Math.sin(time / 120) * 1.5, 0, Math.PI * 2);
  ctx.fill();
  ...
  window.renderTopDownMap = function (ctx, canvas) {
    ...
    const BIOME_PALETTES = [
          // Sector 0 (Whispering Woods): Overgrown Forest Cobblestone
          {
            stone1: "#223326",
            stone2: "#1d2e21",
            stone3: "#17261b",
          ...
          // Sector 1 (Mountain Peaks): Cold Granite & Frost Slate
          {
            stone1: "#222a38",
            stone2: "#1d2532",
            stone3: "#161c28",
          ...
          // Sector 2 (Inferno Depths): Charred Basalt & Molten Magma
          {
            stone1: "#241612",
            stone2: "#1f110c",
            stone3: "#170a07",
          ...
          // Sector 3 (Fungal Swamp): Murky Peat & Spore Sludge
          {
            stone1: "#16281e",
            stone2: "#122117",
            stone3: "#0d1a11",
          ...
          // Sector 4+ (Void Singularity): Levitating Obsidian & Cosmic Runes
          {
            stone1: "#180d2b",
            stone2: "#130923",
            stone3: "#0e051b",
    ...
    for (let r = startRow; r <= endRow; r++) {
      for (let c = startCol; c <= endCol; c++) {
        ...
        if (tile === window.TILE_TYPES.VOID) {
          ...
          if (isHub) {
            // Dressed Sanctuary Masonry Walls for Adventurer's Hub
            let isSouthFloor =
              r + 1 < map.height &&
            ...
            if (isSouthFloor) {
              ...
              ctx.fillRect(px, py, tileSize, tileSize - 8);

              ctx.fillStyle = "#3b4356";
              ctx.fillRect(px, py, tileSize, 2);

              ctx.fillStyle = "#d4af37"; // Gilded wall crown trim
              ctx.fillRect(px, py + tileSize - 10, tileSize, 2);
            ...
            if ((c + r) % 2 === 0) {
              ctx.moveTo(px + 16, py);
              ctx.lineTo(px + 16, midY);
            } else {
              ctx.moveTo(px + 16, midY);
              ctx.lineTo(px + 16, py + tileSize);
            }
            ctx.stroke();
          } else {
            // Enhanced 3D Biome Masonry Wall with Depth & Overhang
            let isSouthFloor =
              r + 1 < map.height &&
              map.grid[r + 1][c] !== window.TILE_TYPES.WALL &&
              map.grid[r + 1][c] !== window.TILE_TYPES.VOID;
            ...
            if (isSouthFloor) {
              ...
              // Front Facing Overhang Shadow Face
              ctx.fillStyle = biome.wallFace;
              ctx.fillRect(px, py + tileSize - 8, tileSize, 8);

              // Wall Base Skirting Shadow
              ctx.fillStyle = biome.wallShadow;
              ctx.fillRect(px, py + tileSize - 2, tileSize, 2);

              // Masonry Horizontal Seam
              ctx.strokeStyle = biome.wallMortar;
              ctx.lineWidth = 1;
              ctx.beginPath();
              ctx.moveTo(px, py + tileSize - 8);
              ctx.lineTo(px + tileSize, py + tileSize - 8);
              ctx.stroke();
            } else {
              // Full Solid Interior Wall
              ctx.fillStyle = biome.wallCap;
              ctx.fillRect(px, py, tileSize, tileSize);

              ctx.strokeStyle = biome.wallShadow;
              ctx.lineWidth = 1;
              ctx.strokeRect(px + 0.5, py + 0.5, tileSize - 1, tileSize - 1);
            ...
            // Masonry Brick Joints with Staggered Pattern
            ctx.strokeStyle = biome.wallMortar;
            ctx.lineWidth = 1;
            ...
            if (tileHash > 0.65) {
              ...
              if (isSouthFloor) {
                ctx.fillRect(ax, py + tileSize - 8, 2, 4);
              }
            }
          }
        } else {
          if (isHub) {
            ...
            // Check if tile is part of the central cross pathways connecting stations
            let isPathway =
              (c === spX && r >= 4 && r <= map.height - 5) ||
              (r === spY && c >= 5 && c <= map.width - 6);

            // Check if tile belongs to a station dais (3x3 area around station)
            ...
            if (stationTile) {
              // Custom architectural station stone paving
              let dx = c - stationTile.x;
              let dy = r - stationTile.y;
              let isAlt = (c + r) % 2 === 0;

              if (stationTile.type === window.TILE_TYPES.STATION_FORGE) {
                              // Heat-Charred Industrial Iron-Grate Paving
                              let time = Date.now();
                              let heatPulse = Math.sin(time / 250) * 0.15 + 0.85;

                              ctx.fillStyle = isAlt ? "#1a1210" : "#140e0c";
                              ctx.fillRect(px, py, tileSize, tileSize);

                              // Under-grate glowing ember ash fissure
                              ctx.fillStyle = `rgba(234, 88, 12, ${0.12 * heatPulse})`;
                              ...
                              ctx.beginPath();
                              ctx.moveTo(px + 8, py);
                              ctx.lineTo(px + 8, py + tileSize);
                              ctx.moveTo(px + 16, py);
                              ctx.lineTo(px + 16, py + tileSize);
                              ctx.moveTo(px + 24, py);
                              ctx.lineTo(px + 24, py + tileSize);
                              ctx.stroke();

                              // Reinforced Corner Rivets
                              ctx.fillStyle = "#b45309";
                              ctx.fillRect(px + 2, py + 2, 3, 3);
                              ctx.fillRect(px + tileSize - 5, py + 2, 3, 3);
                              ctx.fillRect(px + 2, py + tileSize - 5, 3, 3);
                              ctx.fillRect(px + tileSize - 5, py + tileSize - 5, 3, 3);
                            } else if (
                              stationTile.type === window.TILE_TYPES.STATION_PORTAL
                            ) {
                              // Tiered Astral Dais Paving with Floor Runes
                              let time = Date.now();
                              let pulse = Math.sin(time / 400) * 0.15 + 0.85;

                              ctx.fillStyle = isAlt ? "#1e0b2e" : "#160724";
                              ctx.fillRect(px, py, tileSize, tileSize);
                              ...
                              // Runic Floor Arc Inlays (Relative to station center)
                              let relC = c - stationTile.x;
                              let relR = r - stationTile.y;
                              let relDist = Math.hypot(relC, relR);
                              ...
                              if (relDist <= 1.2) {
                                ...
                                ctx.beginPath();
                                ctx.arc(
                                  px + (relC < 0 ? tileSize : relC > 0 ? 0 : tileSize / 2),
                                  py + (relR < 0 ? tileSize : relR > 0 ? 0 : tileSize / 2),
                                  tileSize * 0.8,
                                  0,
                                  Math.PI * 2,
                                );
                              ...
                              if (Math.abs(relC) === 1 && Math.abs(relR) === 1) {
                                ctx.fillStyle = "#d4af37";
                                let cornerX = relC < 0 ? px + 2 : px + tileSize - 6;
                                let cornerY = relR < 0 ? py + 2 : py + tileSize - 6;
                                ctx.fillRect(cornerX, cornerY, 4, 4);
                              }
                            } else if (
                              stationTile.type === window.TILE_TYPES.STATION_ENCHANT
                            ) {
                              // Tiered Astral Lapis Dais Paving with Floor Runes
                              let time = Date.now();
                              let pulse = Math.sin(time / 300) * 0.15 + 0.85;

                              ctx.fillStyle = isAlt ? "#0f0d22" : "#090817";
                              ctx.fillRect(px, py, tileSize, tileSize);

                              // Astral cyan mortar grid
                              ctx.strokeStyle = "rgba(0, 210, 255, 0.25)";
                              ctx.lineWidth = 1;
                              ctx.strokeRect(px + 0.5, py + 0.5, tileSize - 1, tileSize - 1);

                              // Runic Floor Arc Inlays
                              let relC = c - stationTile.x;
                              let relR = r - stationTile.y;
                              let relDist = Math.hypot(relC, relR);

                              if (relDist <= 1.2) {
                                ctx.strokeStyle = `rgba(0, 210, 255, ${0.22 * pulse})`;
                                ctx.lineWidth = 1.2;
                                ctx.beginPath();
                                ctx.arc(
                                  px + (relC < 0 ? tileSize : relC > 0 ? 0 : tileSize / 2),
                              ...
                              // Cyan Corner Trim Accents
                              if (Math.abs(relC) === 1 && Math.abs(relR) === 1) {
                                ctx.fillStyle = "#00d2ff";
                                let cornerX = relC < 0 ? px + 2 : px + tileSize - 4;
                                let cornerY = relR < 0 ? py + 2 : py + tileSize - 4;
                                ctx.fillRect(cornerX, cornerY, 2, 2);
                              }
                            } else if (stationTile.type === window.TILE_TYPES.STATION_STASH) {
                              // Cobalt Slate Paving with Protective Ward Grid
                              let time = Date.now();
                              let wardPulse = Math.sin(time / 300) * 0.15 + 0.85;

                              ctx.fillStyle = isAlt ? "#0c1b2d" : "#081322";
                              ctx.fillRect(px, py, tileSize, tileSize);
                              ...
                              // Runic Security Ward Inlay Lines
                              let relC = c - stationTile.x;
                              let relR = r - stationTile.y;
                              let relDist = Math.hypot(relC, relR);

                              if (relDist <= 1.2) {
                                ctx.strokeStyle = `rgba(56, 189, 248, ${0.2 * wardPulse})`;
                                ctx.lineWidth = 1.2;
                                ctx.beginPath();
                                ctx.arc(
                                  px + (relC < 0 ? tileSize : relC > 0 ? 0 : tileSize / 2),
                              ...
                              ctx.fillRect(px + tileSize - 4, py + 2, 2, 2);
                              ctx.fillRect(px + 2, py + tileSize - 4, 2, 2);
                              ctx.fillRect(px + tileSize - 4, py + tileSize - 4, 2, 2);
                            } else if (stationTile.type === window.TILE_TYPES.STATION_INN) {
                              // Forest Slate Paving with Healing Ward Inlays
                              let time = Date.now();
                              let healPulse = Math.sin(time / 350) * 0.15 + 0.85;

                              ctx.fillStyle = isAlt ? "#0a3328" : "#06241a";
                              ctx.fillRect(px, py, tileSize, tileSize);
                              ...
                              // Organic Moss Patch along tile corners
                              let tileHash =
                                Math.abs(Math.sin(c * 17.1 + r * 31.3) * 43758.5453) % 1.0;
                              if (tileHash > 0.4) {
                              ...
                              // Runic Healing Circle Inlay
                              let relC = c - stationTile.x;
                              let relR = r - stationTile.y;
                              let relDist = Math.hypot(relC, relR);

                              if (relDist <= 1.2) {
                                ctx.strokeStyle = `rgba(52, 211, 153, ${0.22 * healPulse})`;
                                ctx.lineWidth = 1.2;
                                ctx.beginPath();
                                ctx.arc(
                                  px + (relC < 0 ? tileSize : relC > 0 ? 0 : tileSize / 2),
              ...
              // Soft Slate Mortar Lines
              ctx.strokeStyle = isPathway
                ? "rgba(255, 255, 255, 0.07)"
                : "rgba(255, 255, 255, 0.03)";
              ctx.lineWidth = 1;
              ctx.strokeRect(px + 0.5, py + 0.5, tileSize - 1, tileSize - 1);

              // Pathway Brass Edge Inlays
              if (isPathway) {
                ctx.strokeStyle = "rgba(212, 175, 55, 0.15)";
                ctx.lineWidth = 1;
                if (c === spX - 1) {
              ...
              // Central Spawn Point: Polished Gold & Brass Compass Rose
              if (c === spX && r === spY) {
                ...
                ctx.lineWidth = 2;
                ctx.beginPath();
                ctx.arc(spCx, spCy, 14, 0, Math.PI * 2);
                ctx.stroke();
                ...
                ctx.beginPath();
                ctx.moveTo(spCx, spCy - 12);
                ctx.lineTo(spCx + 2.5, spCy - 2.5);
                ctx.lineTo(spCx + 12, spCy);
                ctx.lineTo(spCx + 2.5, spCy + 2.5);
                ctx.lineTo(spCx, spCy + 12);
                ctx.lineTo(spCx - 2.5, spCy + 2.5);
                ctx.lineTo(spCx - 12, spCy);
                ctx.lineTo(spCx - 2.5, spCy - 2.5);
                ctx.closePath();
                ...
                ctx.beginPath();
                ctx.arc(spCx, spCy, 2.5, 0, Math.PI * 2);
                ctx.fill();
                ctx.restore();
              }
            }
          } else {
            // Sector-Themed Organic Seamless Flagstone & Ground Blending Engine
                        // 1. Continuous Base Stone Fill with World-Space Trigonometric Noise
                        ...
                        // Deterministic Layout Variation (4 distinct sub-stone patterns based on tile position)
                        let layoutType = (c * 7 + r * 13) % 4;

                        // Helper to render an individual bevel-shaded flagstone slab extending edge-to-edge
                        let drawFlagstone = (sx, sy, sw, sh, colorIdx) => {
                          ...
                          ctx.fillStyle = biome.stoneHighlight;
                          ctx.fillRect(sx, sy, sw, 1);
                          ctx.fillRect(sx, sy, 1, sh);

                          // Soft Inner Drop Shadow Edge
                          ctx.fillStyle = biome.stoneShadow;
                          ctx.fillRect(sx, sy + sh - 1, sw, 1);
                          ctx.fillRect(sx + sw - 1, sy, 1, sh);
                        };

                        // Render Sub-Divided Irregular Flagstone Slabs (Full Edge-to-Edge to Eliminate Tile Border Lines)
                        if (layoutType === 0) {
                          drawFlagstone(px, py, tileSize, 18, (c + r) % 3);
                          drawFlagstone(px, py + 18, 15, tileSize - 18, (c * 2 + r) % 3);
                          drawFlagstone(px + 15, py + 18, tileSize - 15, tileSize - 18, (c + r * 2) % 3);
                        } else if (layoutType === 1) {
                          drawFlagstone(px, py, 14, tileSize, (c + r * 3) % 3);
                          drawFlagstone(px + 14, py, tileSize - 14, 15, (c * 3 + r) % 3);
                          drawFlagstone(px + 14, py + 15, tileSize - 14, tileSize - 15, (c + r) % 3);
                        } else if (layoutType === 2) {
                          drawFlagstone(px, py, 16, 14, (c + r) % 3);
                          drawFlagstone(px + 16, py, tileSize - 16, 14, (c * 2 + r * 2) % 3);
                          drawFlagstone(px, py + 14, tileSize, tileSize - 14, (c + r * 3) % 3);
                        } else {
                        ...
                        // 2. Internal Internal-Only Mortar Cracks (Only drawn INSIDE tiles to break up uniform slabs, not on outer grid boundaries)
                        ctx.strokeStyle = biome.seamGlow;
                        ctx.lineWidth = 0.8;
                        ctx.beginPath();
                        if (layoutType === 0) {
                          ctx.moveTo(px, py + 18);
                          ctx.lineTo(px + tileSize, py + 18);
                          ctx.moveTo(px + 15, py + 18);
                          ctx.lineTo(px + 15, py + tileSize);
                        } else if (layoutType === 1) {
                          ctx.moveTo(px + 14, py);
                          ctx.lineTo(px + 14, py + tileSize);
                          ctx.moveTo(px + 14, py + 15);
                          ctx.lineTo(px + tileSize, py + 15);
                        } else if (layoutType === 2) {
                          ctx.moveTo(px, py + 14);
                          ctx.lineTo(px + tileSize, py + 14);
                          ctx.moveTo(px + 16, py);
                          ctx.lineTo(px + 16, py + 14);
                        } else {
                          ctx.moveTo(px + 17, py);
                          ctx.lineTo(px + 17, py + tileSize);
                          ctx.moveTo(px, py + 16);
                          ctx.lineTo(px + 17, py + 16);
                        }
                        ctx.stroke();

            // 3. Ambient Occlusion Wall Skirt Shadows (Grounds walls onto floor)
            if (r > 0 && map.grid[r - 1][c] === window.TILE_TYPES.WALL) {
              let northShadow = ctx.createLinearGradient(px, py, px, py + 12);
              northShadow.addColorStop(0, "rgba(0, 0, 0, 0.65)");
              northShadow.addColorStop(1, "rgba(0, 0, 0, 0)");
              ctx.fillStyle = northShadow;
              ctx.fillRect(px, py, tileSize, 12);
            }
            if (c > 0 && map.grid[r][c - 1] === window.TILE_TYPES.WALL) {
              let westShadow = ctx.createLinearGradient(px, py, px + 8, py);
              westShadow.addColorStop(0, "rgba(0, 0, 0, 0.45)");
              westShadow.addColorStop(1, "rgba(0, 0, 0, 0)");
              ctx.fillStyle = westShadow;
              ctx.fillRect(px, py, 8, tileSize);
            }

            // 4. Sector-Specific Floor Micro-Details & Textures
            if (tileHash > 0.82) {
              // Biome Accent Scatter / Fallen Leaves / Embers / Spores
              ctx.fillStyle = biome.accent;
              let ax = px + 4 + ((tileHash * 19) % 22);
              let ay = py + 4 + ((tileHash * 29) % 22);
              ctx.fillRect(ax, ay, 2.5, 2.5);
              ctx.fillRect(ax + 3, ay - 2, 1.8, 1.8);
            } else if (tileHash > 0.62) {
              // Jagged Stone Fissure / Fine Fracture
    ...
    for (let r = startRow; r <= endRow; r++) {
      for (let c = startCol; c <= endCol; c++) {
        ...
        if (tile === window.TILE_TYPES.EXTRACTION_ZONE) {
            ...
            window.drawDungeonStructureTile = function (ctx, tileType, px, py, tileSize) {
              if (tileType === window.TILE_TYPES.CHEST_SPAWN) {
                ...
                let waterGrad = ctx.createRadialGradient(
                  cx,
                  cy + 4,
                  2,
                  cx,
  ...
  window.renderMinimap = function (ctx, canvas) {
      ...
      for (let r = 0; r < map.height; r++) {
        for (let c = 0; c < map.width; c++) {
          let isExplored = map.exploredGrid && map.exploredGrid[r] && map.exploredGrid[r][c];
      ...
      if (map.portalDiscovered && map.extractionTile) {
        ...
        let isExplored = map.exploredGrid && map.exploredGrid[pTileY] && map.exploredGrid[pTileY][pTileX];
        if (isExplored) {
